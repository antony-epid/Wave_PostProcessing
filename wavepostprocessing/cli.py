...
def main():
    ...
    if config.get("processing").lower() == "pampro":
        if config.get("run_pampro_merge_metafiles").lower() == 'yes':
            ...
            jid = submit_jobs("pampro_merge_metafiles.py", args.directory, arrsize=1,num_cpu=1, jid=jid, budgacc=mybudgacc)
        if config.get("run_pampro_collate_anomalies").lower()  == 'yes':
            print_message("Collating Anomalies")
            jid = submit_jobs("pampro_collate_anomalies.py", args.directory,  arrsize=1,num_cpu=1, jid=jid, budgacc=mybudgacc)

    if config.get("run_filelist_generation").lower()  == 'yes':
        print_message("Creating Filelist")
        jid = submit_jobs("filelist_generation.py", args.directory,  arrsize=1,num_cpu=1, jid=jid, budgacc=mybudgacc)
