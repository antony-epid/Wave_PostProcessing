...
def main():
    ...
    parser.add_argument("directory", nargs="?", default=".", help="Directory containing config.yaml")
    ...
    if config.get("run_generic_exh_postprocessing").lower()  == 'yes':
        ...
        jid = submit_jobs("wavepostprocessing.generic_exh_postprocessing", args.directory, arrsize=10,num_cpu=1, jid=jid, budgacc=mybudgacc)
    ...
    if config.get("run_collapse_results_to_summary").lower() == 'yes' or config.get("run_collapse_results_to_daily").lower() == 'yes':
        ...
        jid = submit_jobs("wavepostprocessing.collapse_results", args.directory, arrsize=10,num_cpu=1, jid=jid, budgacc=mybudgacc)
    ...
    if config.get("run_prepare_summary_release").lower() == 'yes' or config.get("run_prepare_daily_release").lower() == 'yes' or config.get("run_prepare_hourly_release").lower() == 'yes' or  config.get("run_prepare_minute_level_release").lower() == 'yes':
        ...
        jid = submit_jobs("wavepostprocessing.prepare_releases", args.directory,  arrsize=1,num_cpu=1, jid=jid, budgacc=mybudgacc)

    print_message(Fore.BLUE + "WaveProcessing completed successfully.")
