############################################################################################################
# Appending individual Hourly files Together
# Author: CAS
# Date: 03/07/2024 (started)
...
############################################################################################################
# PART C: This do file will append together all individual Hourly files as well as join the files which have not produced an individual daily file
############################################################################################################

# CREATING A FILE LIST OF ALL FILES IN THE INDIVIDUAL TRIMMED FOLDER #
def create_filelist():

    # DELETE ALL PAST FILELISTS IN INDIVIDUAL TRIMMED FOLDER
    TRIMMED_FOLDER_PATH = os.path.join(config.ROOT_FOLDER, config.RESULTS_FOLDER, config.SUMMARY_FOLDER, config.INDIVIDUAL_TRIMMED_F, config.TIME_RES_FOLDER)

    if os.path.exists(os.path.join(TRIMMED_FOLDER_PATH, "filelist.txt")):  # If any filelist in the daily folder, this will be deleted.
        os.remove(os.path.join(TRIMMED_FOLDER_PATH, "filelist.txt"))
...
def appending_files(files_list):
    ...
    for file_name in files_list:
        file_path = os.path.join(config.ROOT_FOLDER, config.RESULTS_FOLDER, config.SUMMARY_FOLDER, config.INDIVIDUAL_TRIMMED_F, config.TIME_RES_FOLDER, f"{file_name}")
    ...
    appended_df = appended_df.dropna(subset=['file_id'])
...
def appending_no_analysis_files(no_analysis_filelist, appended_df):
    ...
    if not no_analysis_filelist:
        ...
        file_name = os.path.join(output_file_path, f"{config.HOUR_OUTPUT_FILE}.csv")
