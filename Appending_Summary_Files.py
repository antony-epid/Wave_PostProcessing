...
def create_filelist():
    ...
    SUMMARY_FOLDER_PATH = os.path.join(config.ROOT_FOLDER, config.RESULTS_FOLDER, config.SUMMARY_FOLDER, config.INDIVIDUAL_SUM_F, config.TIME_RES_FOLDER)

    if os.path.exists(os.path.join(SUMMARY_FOLDER_PATH, "filelist.txt")):  # If any filelist in the summary folder, this will be deleted.
        os.remove(os.path.join(SUMMARY_FOLDER_PATH, "filelist.txt"))
...
def remove_files():
    ...
    filelist_df = filelist_df[~filelist_df['file_name'].str.contains(config.SUM_OUTPUT_FILE, case=False, na=False)]
...
def appending_files(files_list):
    ...
    for file_name in files_list:
        file_path = os.path.join(config.ROOT_FOLDER, config.RESULTS_FOLDER, config.SUMMARY_FOLDER, config.INDIVIDUAL_SUM_F, config.TIME_RES_FOLDER, f"{file_name}")
...
def appending_no_analysis_files(no_analysis_filelist, appended_df):
    ...
    if not no_analysis_filelist:
        ...
        file_name = os.path.join(output_file_path, f"{config.SUM_OUTPUT_FILE}.csv")
